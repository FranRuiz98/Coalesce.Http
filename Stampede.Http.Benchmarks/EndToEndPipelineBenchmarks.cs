...
///   • Stampede.Http full pipeline (CachingMiddleware → CoalescingHandler → backend)
///
...
public class EndToEndPipelineBenchmarks
{
    ...
    public async Task Setup()
    {
        ...
        // ── Stampede.Http pipeline: CachingMiddleware → CoalescingHandler → backend ──
    ...
    [Benchmark(Description = "Stampede.Http pipeline (cache hits after first request)")]
    public async Task StampedeHttpPipeline()
    {
    ...
    [Benchmark(Description = "Stampede.Http concurrent burst (coalescing + cache)")]
    public async Task StampedeHttpConcurrentBurst()
    {
