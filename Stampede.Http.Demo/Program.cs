...
{
    Banner("Stampede.Http — Library Overview");
    ...
    AnsiConsole.MarkupLine("[dim]  Nine instruments emitted under the [/][cyan]\"Stampede.Http\"[/][dim] meter (see Demo 7).[/]");
}
...
static async Task Demo5_PerRequestPoliciesAsync()
{
    ...
    Note("All Stampede.Http behaviours can be overridden on individual requests via");
...
static async Task Demo7_MetricsAsync()
{
    ...
    Note($"Meter name : \"{StampedeHttpMetrics.MeterName}\"");
    Note("Integrate with any OpenTelemetry exporter, Prometheus, or a plain MeterListener.");
    Note("");
    Note("Instruments (all under the \"stampede_http.\" prefix):");
    ...
    {
        ["stampede_http.cache.hits"] = "Scenario B: 2 sequential hits",
        ["stampede_http.cache.misses"] = "Scenario A: all 5 concurrent missed",
        ["stampede_http.cache.revalidations"] = "no conditional requests in this demo",
        ["stampede_http.cache.stale_errors_served"] = "see Demo 4",
        ["stampede_http.cache.stale_while_revalidate_served"] = "see Demo 3",
        ["stampede_http.cache.invalidations"] = "see Demo 6",
        ["stampede_http.coalescing.deduplicated"] = "Scenario A: 4 of 5 concurrent were waiters",
        ["stampede_http.coalescing.inflight"] = "all requests completed, 0 in-flight",
        ["stampede_http.coalescing.timeouts"] = "no timeout configured",
    };
    ...
    foreach (var (name, value) in counters.OrderBy(kv => kv.Key))
    {
        ...
        string shortName = name["stampede_http.".Length..];
        table.AddRow(
            $"[cyan]stampede_http.[/]{shortName}",
            $"[green]{value}[/]",
            $"[dim]{desc ?? ""}[/]"
        );
    }
    ...
    Action<CoalescerOptions>? coalesceOpts = null)
{
    ...
    services.AddHttpClient("demo")
        .AddStampedeHttp(cacheOpts, coalesceOpts)
