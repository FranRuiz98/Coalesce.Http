...
namespace Stampede.Http.Tests.Metrics;

/// <summary>
/// Verifies that <see cref="StampedeHttpMetrics"/> emits the correct instrument readings
/// for cache hits, misses, revalidations, stale-if-error, and coalescing deduplication.
/// </summary>
public sealed class StampedeHttpMetricsTests : IDisposable
{
    private readonly StampedeHttpMetrics _metrics;
    ...
    public StampedeHttpMetricsTests()
    {
        _metrics = new StampedeHttpMetrics();
        ...
        _listener.SetMeasurementEventCallback<long>((instrument, measurement, tags, _) =>
        {
            ...
            {
                case "stampede_http.cache.hits":
                    _cacheHits += measurement;
                    if (isHead) _headCacheHits += measurement;
                    break;
                case "stampede_http.cache.misses":          _cacheMisses += measurement; break;
                case "stampede_http.cache.revalidations":
                    _cacheRevalidations += measurement;
                    if (isHead) _headCacheRevalidations += measurement;
                    break;
                case "stampede_http.cache.stale_errors_served": _staleErrorsServed += measurement; break;
                case "stampede_http.coalescing.deduplicated":   _coalescedDeduplicated += measurement; break;
                case "stampede_http.coalescing.inflight":       _coalescedInflight += measurement; break;
                case "stampede_http.coalescing.timeouts":       _coalescedTimeouts += measurement; break;
            }
        });
    ...
    {
        StampedeHttpMetrics.MeterName.Should().Be("Stampede.Http");
    }
